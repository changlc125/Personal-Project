...
class VWAP(object):
    ...
    def run(self):
        ...
        #end TODO
        typical_price = (self.ohlcv_df['close']+self.ohlcv_df['high']+self.ohlcv_df['low'])/3
#         vwap = np.average(typical_price,weights=self.ohlcv_df['volume'])
        Cum_Vol = self.ohlcv_df['volume'].cumsum()
        Cum_Vol_Price = (self.ohlcv_df['volume'] * typical_price).cumsum()
        self.vwap = Cum_Vol_Price / Cum_Vol
        try:
            zero_or_not=Cum_Vol_Price / Cum_Vol
...
#         RS_up = up_days.rolling(window,min_periods=1).mean()
#         RS_down = down_days.rolling(window,min_periods=1).mean()
        RS_up = up_days.ewm(com=13,adjust=False, min_periods=14).mean()
        RS_down = down_days.ewm(com=13,adjust=False, min_periods=14).mean()
        
        try:
            zero_or_not=RS_up/RS_down
        except ZeroDivisionError:
            print ("divide by zero")
